...
! assign alpha_c variable labor shares
alpha(0:13) = alpha_DATA
! assume stays the same after t=13 (2015)
do it = 14,TT
...
subroutine prices(it)  
	...
	if(smopec)then
		...
		w(it) = A_c(it) * (1d0 - alpha(it))*(max(KK_C(it),0.0000001d0) &
		& /max(LL_C(it),0.0000001d0))**alpha(it)
	...
	!	write(*,*) 'pop retired:',sum(pop(JR:JJ,it))
		...
		! normalize aggregate consumption TFP
		w(it) = A_c(it) * (1d0 - alpha(it))*(max(KK_C(it),0.000000001d0) &
		& /max(LL_C(it),0.000000001d0))**alpha(it)
		r(it) = A_c(it) * alpha(it) *(max(KK_C(it),0.000000001d0) &
		& /max(LL_C(it),0.000000001d0))**(alpha(it) - 1d0)
...
subroutine aggregation(it_in)
	...
	! marginal costs
	MC_c = (1d0/A_c(it)) * (r(it)/alpha(it))**alpha(it) * (w(it)/(1d0-alpha(it)))**(1d0-alpha(it))
	MC_h = (1d0/A_h(it)) * (r(it)/alpha_h)**alpha_h *(w(it)/(1d0-alpha_h))**(1d0-alpha_h)
   
	! capital split
	if((MC_c * alpha(it) * (CC(it) + II(it)) + MC_h * alpha_h * HH(it)) > 0d0)then
		KK_C(it) = MC_c * alpha(it) * (CC(it) + II(it)) &
		& / (MC_c * alpha(it) * (CC(it) + II(it)) + MC_h * alpha_h * HH(it)) * KK(it)
	else
		KK_C(it) = 0d0
	endif
    ...
    ! labor split
    if((MC_c * (1d0-alpha(it)) * (CC(it) + II(it)) + MC_h * (1d0-alpha_h) * HH(it)) > 0d0)then
		LL_C(it) = MC_c * (1d0-alpha(it)) * (CC(it) + II(it)) &
		& / (MC_c * (1d0-alpha(it)) * (CC(it) + II(it)) + MC_h * (1d0-alpha_h) * HH(it)) * LL(it)
	else
		LL_C(it) = 0d0
	endif
...
!	YY(it) = CC(it) + II(it) + HH(it)
	YY(it) = A_c(it)*max(KK_C(it),0.0000001d0)**alpha(it)*max(LL_C(it),0.0000001d0)**(1d0-alpha(it)) &
